...
- [Фреймворк для настройки ботов](#фреймворк-для-настройки-ботов)
    ...
    - [Пример](#пример)
    - [Предложение своих идей](#предложение-своих-идей)
    - [Лицензия](#лицензия)
...
* **Контекстная обработка** — каждое обновление обрабатывается с контекстом, который содержит данные, специфичные для
  запроса
* **Поддержка Middleware** — создание промежуточных обработчиков для логирования, аутентификации, ограничения частоты
...
3. Выполните команду в терминале, чтобы запустить бота:

```go
go run main.go
```

После чего в консоли отобразится сообщение: `Бот запускается...`

4. Откройте диалог с вашим ботом в MAX и отправьте команду `/start`. Бот ответит вам приветственным
   сообщением:
...
Используйте метод `Handle` для регистрации обработчика необходимых событий:
...
Используйте метод `Handle` для регистрации обработчика для необходимых команд:
...
Используйте метод `Handle` для регистрации обработчика необходимых сообщений:
...
Используйте метод `Handle` для регистрации обработчика callback-запросов:
...
## Предложение своих идей

Чтобы предложить идеи своих доработок, отправляйте `Pull Request` в проект:
